            ...
            View Class Averages
						...
						if(isset($_GET['Num'])){
							...
							if($_SESSION['class']==3 || $_SESSION	['class']==4){
								$db1->query('SELECT *  FROM averageTotals where  exam=:exam AND Class=:class AND term=:term AND year=:Year ORDER BY points DESC');
								...
								//print_r($pos);
								foreach ($pos as $keyp) {
									$db1->query("UPDATE averageTotals SET position=:position WHERE  id=:id");
							...
							$db1->query('SELECT *  FROM averageTotals where  exam=:exam AND Class=:class AND term=:term AND year=:Year ORDER BY total DESC');
							//$db1->bind(':marks',100);
							$db1->bind(':exam',$_GET['Num']);
							$db1->bind(':class',$_GET['Class']);
							$db1->bind(':term',$_GET['Term']);
						...
						?>
						<form method="POST" action="classAverage.php">
							<?php echo $Msg;?>
							<fieldset>
						...
						?>
            <a href="printAverage.php?Exam=<?php echo $_SESSION['exam'];?>&Class=<?php echo $_SESSION['class'];?>&Term=<?php echo $_SESSION['term'];?>&Year=<?php echo $_SESSION['year'];?>" class="badge badge-success fa fa-print" style="float:right;margin-bottom:5px;" >&nbsp;Print</a>&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="classAverage.php?Trun=<?php echo $_SESSION['class'];?>" class="badge badge-danger fa fa-times-circle" style="float:right;margin-bottom:5px;" >&nbsp;Error Ranking?</a>&nbsp;&nbsp;
						<a href="classAverage.php?Num=<?php echo $_SESSION['exam'];?>&Class=<?php echo $_SESSION['class'];?>&Term=<?php echo $_SESSION['term'];?>&Year=<?php echo $_SESSION['year'];?>" class="badge badge-info fa fa-arrow-up" style="float:right;margin-bottom:5px;" >&nbsp;Generate Numbers</a>&nbsp;&nbsp;&nbsp;
                    ...
                    ?>
                      ...
                      <?php
											...
											$totalPoints=0;
											foreach ($subjects as $key ) {
                        $cat1=$db->displayCat1($key1->regNo,'CAT 1',$_SESSION['year'],$_SESSION['term'],$key->subjectName);
                        $cat2=$db->displayCat2($key1->regNo,'CAT 2',$_SESSION['year'],$_SESSION['term'],$key->subjectName);
                        ...
                        $subjectCat=$db->getCategory($key->subjectName);
												$grade=$db->getGrade($subjectCat,$mark);
                        ...
                        if($sp=="--"){
                          ...
                          $totalPoints=$totalPoints+$sp;
												...
												$stream=$db->getStream($key1->regNo);
												$db->addAverage($_SESSION['exam'],$_SESSION['term'],$_SESSION['class'],$stream,$_SESSION['year'],$key1->regNo,$key->subjectName,$mark,$grade);
											...
											$isUpdated=$db->checkAverageExam($_SESSION['exam'],$_SESSION['class'],$_SESSION['year'],$_SESSION['term']);
											if($isUpdated){//update examTotals set points=1,Grade='A' where id=4 AND class=1
												$db->query("UPDATE  averageTotals SET total=:total,points=:points,MeanScore=:meanScore,Grade=:grade WHERE studentReg=:studentReg AND exam=:exam AND Class=:class AND Stream=:stream AND term=:term AND year=:year");
												...
												$db->query("INSERT INTO averageTotals(studentReg,exam,total,points,MeanScore,Grade,Class,Stream,term,year) VALUES(:studentReg,:exam,:total,:points,:meanScore,:grade,:class,:stream,:term,:year)");
												$db->bind(':studentReg',$key1->regNo);
												$db->bind(':exam',$_SESSION['exam']);
												$db->bind(':total',$markx);
												$points=round($markx/$count);
								...
								if($_SESSION['exam'] == "" || $_SESSION['class']==""){
									...
									$notEmpty=$db->checkAverageExam($_SESSION['exam'],$_SESSION['class'],$_SESSION['year'],$_SESSION['term']);
									if($notEmpty){
										//just do nothing
									}else{
									$db->query("INSERT INTO isAvPosted(exam,class,year,term) VALUES(:exam,:class,:year,:term)");
									$db->bind(':exam',$_SESSION['exam']);
									$db->bind(':class',$_SESSION['class']);
									$db->bind(':year',$_SESSION['year']);
