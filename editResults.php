...
if(isset($_POST['sub'])){
	...
	$db=new Database();
	$db->query("UPDATE examResults SET marks=:marks WHERE id=:id");
	$db->bind(':marks',$marks);
  $db->bind(':id',$rId);
	if($db->execute()){
...
?>
            ...
            <?php
            $db=new Database();
            $db->query("select * from examResults where id=:id");
            $db->bind(':id',$id);
            if($results=$db->single()){
            ?>
					<form method="POST" action="editResults.php" enctype="multipart/form-data">
						<?php echo $Msg;?>
						<div class="row">
							<div class="col-sm-6">
