  ...
  $db=new Database();
    ...
    function _endpage()
    {
      ...
      parent::_endpage();
		...
		function head($db,$class,$stream,$subject){
      $detail=$db->getSchool();
      foreach($detail as $key){
        ...
        $this->SetFont('Courier','B','10');
				...
				$this->Ln(10);
        $this->SetFont('Arial','B','10');
				$this->Cell(200,5,"Form ".$class." ".$stream." ".$subject. " MarkSheet",0,0,'C');
				$this->Ln(10);
      }
		}
		function footer(){
      ...
      $this->SetTextColor(125,125,125);
		...
		function headerTable(){
			...
			$this->Cell(10,5,'S/N',1,0,'C');
			$this->Cell(50,5,'Names',1,0,'C');
			$this->Cell(20,5,'Reg. Number',1,0,'C');
      $this->Cell(20,5,"Cat 1",1,0,'C');
      $this->Cell(20,5,"Cat 2",1,0,'C');
      $this->Cell(35,5,"Main Exam",1,0,'C');
      $this->Cell(35,5,"Total x/100%",1,0,'C');
			$this->Ln();
		}
      ...
      if($_SESSION['stream']=="All Streams"){
        ...
        $db->query("SELECT * FROM student WHERE class=:class AND stream=:stream AND status=:status ORDER BY regNo ASC");
        $db->bind(':class',$_SESSION['class']);
      ...
      foreach ($results as $data) {
        $this->Cell(10,5,$count,1,0,'C');
        $this->SetFont('Arial','',8);
        $this->Cell(50,5,$data->names,1,0,'L');
        $this->SetFont('Arial','',10);
				$this->Cell(20,5,$data->regNo,1,0,'C');
      ...
      $this->Cell(150,10,"Total ",0,0,'C');
      $this->Cell(0,10,"===========",0,0,'C');
      $this->Ln(3);
      $this->Cell(150,10,"",0,0,'C');
      $this->Cell(0,10,$total." Students",0,0,'C');
      $this->Ln(3);
      $this->Cell(150,10," ",0,0,'C');
      $this->Cell(0,10,"===========",0,0,'C');
		}
	...
	$pdf->AliasNbPages();
	$pdf->AddPage('p','A4',0);
	$pdf->head($db,$_SESSION['class'],$_SESSION['stream'],$_SESSION['subject']);
	$pdf->headerTable();
	$pdf->viewTable($db);
	$pdf->output();
