            ...
            Manage Exam Results
						...
						if(isset($_GET['Num'])){
							...
							if($_SESSION['class']==3 || $_SESSION	['class']==4){
								$db1->query('SELECT *  FROM catTotals where  exam=:exam AND Class=:class AND term=:term AND year=:Year ORDER BY MeanScore DESC');
								...
								//print_r($pos);
								foreach ($pos as $keyp) {
									$db1->query("UPDATE catTotals SET position=:position WHERE  id=:id");
							...
							$db1->query('SELECT *  FROM catTotals where  exam=:exam AND Class=:class AND term=:term AND year=:Year ORDER BY total DESC');
							//$db1->bind(':marks',100);
							$db1->bind(':exam',$_GET['Num']);
							$db1->bind(':class',$_GET['Class']);
							$db1->bind(':term',$_GET['Term']);
						...
						?>
						<form method="POST" action="resultsCat.php">
							<?php echo $Msg;?>
							<fieldset>
						...
						?>
						<a href="resultsCat.php?Num=<?php echo $_SESSION['exam'];?>&Class=<?php echo $_SESSION['class'];?>&Term=<?php echo $_SESSION['term'];?>&Year=<?php echo $_SESSION['year'];?>" class="badge badge-info fa fa-arrow-up" style="float:right;margin-bottom:5px;" >&nbsp;Generate Numbers</a>&nbsp;&nbsp;&nbsp;
						<a href="PrintCat.php?Exam=<?php echo $_SESSION['exam'];?>&Class=<?php echo $_SESSION['class'];?>&Term=<?php echo $_SESSION['term'];?>&Year=<?php echo $_SESSION['year'];?>" class="badge badge-success fa fa-print" style="float:right;margin-bottom:5px;" >&nbsp;Print</a>&nbsp;&nbsp;&nbsp;&nbsp;
									...
									<tr>
									  ...
									  $results=$database->resultSet();
										...
										?>
												...
												$totalPoints=0;
												foreach ($subjects as $key ) {
													$mark=$db->getCatMarks($key->subjectName,$_SESSION['class'],$_SESSION['exam'],$_SESSION['year'],$key1->regNo);
													$grade=$db->getCatSubjectGrade($key->subjectName,$_SESSION['class'],$_SESSION['exam'],$_SESSION['year'],$key1->regNo);
												...
												if($isUpdated){//update examTotals set points=1,Grade='A' where id=4 AND class=1
													$db->query("UPDATE  catTotals SET total=:total,points=:points,MeanScore=:meanScore,Grade=:grade WHERE studentReg=:studentReg AND exam=:exam AND Class=:class AND Stream=:stream AND term=:term AND year=:year");
													...
													$db->query("INSERT INTO catTotals(studentReg,exam,total,points,MeanScore,Grade,Class,Stream,term,year) VALUES(:studentReg,:exam,:total,:points,:meanScore,:grade,:class,:stream,:term,:year)");
													$db->bind(':studentReg',$key1->regNo);
													$db->bind(':exam',$_SESSION['exam']);
													$db->bind(':total',$markx);
													$points=round($markx/$count);
												...
												<td style='font-size:8px;font-family:courier'><?php
												 ...
												 <td style="font-family:courier;font-size:8px"><?php
														echo $db->getCatPosition($key1->regNo,$_SESSION['exam'],$_SESSION['class'],$term,$_SESSION['year'])
												 ?></td>
											</tr>
									<?php } }}
									if($_SESSION['exam'] == "" || $_SESSION['class']==""){
